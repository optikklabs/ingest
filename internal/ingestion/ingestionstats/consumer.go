...
type Consumer struct {
	...
	writer core.Writer[*schema.StatRow]
	dlq    *core.DLQ
}
...
func (c *Consumer) handle(ctx context.Context, recs []*kgo.Record) error {
	rows := make([]*schema.StatRow, 0, len(recs))
	for _, r := range recs {
		row := &schema.StatRow{}
		if err := proto.Unmarshal(r.Value, row); err != nil {
			slog.WarnContext(ctx, "ingestion_stats consumer: dropped malformed record",
	...
	if err := c.writer.Insert(ctx, rows); err != nil {
		slog.ErrorContext(ctx, "ingestion_stats consumer: CH insert failed → DLQ",
