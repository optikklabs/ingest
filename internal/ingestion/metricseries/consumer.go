...
type Consumer struct {
	...
	writer core.Writer[*schema.SeriesRow]
	dlq    *core.DLQ
}
...
func (c *Consumer) handle(ctx context.Context, recs []*kgo.Record) error {
	rows := make([]*schema.SeriesRow, 0, len(recs))
	for _, r := range recs {
		row := &schema.SeriesRow{}
		if err := proto.Unmarshal(r.Value, row); err != nil {
			slog.WarnContext(ctx, "metric_series consumer: dropped malformed record",
	...
	if err := c.writer.Insert(ctx, rows); err != nil {
		slog.ErrorContext(ctx, "metric_series consumer: CH insert failed → DLQ",
